...
const handleNotificationSent = (result) => {
  ...
  const loadDashboardData = async (userId) => {
    ...
    try {
      ...
      const grossistesResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/users?role_id=3&parent_id=${userId}`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          },
      ...
      const processedGrossistes = grossistesData.map(grossiste => ({
        ...grossiste,
  ...
  // Fonction pour télécharger les données Excel
  ...
  const renderStandardSection = () => (
    <div>
      ...
      {/* Section des grossistes Standard */}
      <div className={`${cardBackgroundClass} shadow rounded-lg p-4 sm:p-6 mb-6`}>
        <div className={`${cardBackgroundClass} shadow rounded-lg p-3 sm:p-4 md:p-6 overflow-hidden`}>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3 sm:gap-0">
            <h2 className={`text-lg sm:text-xl font-semibold ${textClass}`}>Mes grossistes Standard</h2>
            <button 
              onClick={() => setIsModalOpen(true)}
              className="w-full sm:w-auto px-3 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition-colors text-sm"
            >
  ...
  const renderVipSection = () => (
    <div>
      ...
      {/* Section des grossistes VIP */}
      <div className={`${cardBackgroundClass} shadow rounded-lg p-4 sm:p-6 mb-6`}>
        <div className={`${cardBackgroundClass} shadow rounded-lg p-3 sm:p-4 md:p-6 overflow-hidden`}>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-3 sm:gap-0">
            <h2 className={`text-lg sm:text-xl font-semibold ${textClass}`}>Mes grossistes VIP</h2>
            <button 
              onClick={() => setIsModalOpen(true)}
              className="w-full sm:w-auto px-3 py-2 bg-yellow-600 text-white rounded hover:bg-yellow-700 transition-colors text-sm"
            >
              Ajouter un grossiste
            </button>
            <ModalGen
          ...
          {renderGrossistesTable(grossistes, statsVip, true)}
        </div>
      </div>
    </div>
  );
      ...
      <div className="overflow-x-auto">
        <div className="min-w-full overflow-x-auto -mx-4 md:mx-0">
          <Table
            ...
            data={grossistesData.map(grossiste => ({
              ...grossiste,
              photo: (
                <Image 
                  src={grossiste.photo_profil || '/bg.jpg'}
                  alt="Photo de profil"
                  width={40}
                  height={35}
              ...
              tickets_actives: isVip ? statsData?.grossistes_vip?.find(g => g.id === grossiste.id)?.nb_vip_ven || 0 : statsData?.grossistes?.find(g => g.id === grossiste.id)?.nb_tickets_ven || 0,
              tickets_disponibles: isVip ? statsData?.grossistes_vip?.find(g => g.id === grossiste.id)?.nb_vip || 0 : statsData?.grossistes?.find(g => g.id === grossiste.id)?.nb_tickets || 0,
              tickets_remis: isVip ? statsData?.grossistes_vip?.find(g => g.id === grossiste.id)?.nb_vip_reçu || 0 : statsData?.grossistes?.find(g => g.id === grossiste.id)?.nb_tickets_reçu || 0,
              money: isVip ? (statsData?.grossistes_vip?.find(g => g.id === grossiste.id)?.nb_vip_ven || 0) * pricePerTicket : (statsData?.grossistes?.find(g => g.id === grossiste.id)?.nb_tickets_ven || 0) * pricePerTicket,
              ...
              actions: (
                <div className="flex flex-wrap gap-1 justify-center sm:justify-start space-x-2">
                  ...
                  <button 
                    ...
                    className="px-2 py-1 bg-violet-600 hover:bg-blue-700 text-white rounded transition-colors text-xs whitespace-nowrap"
                    title="Modifier">
                    Detail
                  </button>

                  {/* Le composant modal doit être hors du bouton */}
                  <NotificationModal
            ...
            emptyMessage={`Aucun grossiste disponible`}
          />
        </div>
      </div>
    ) : (
      ...
      <div className="flex flex-1">
        ...
        <main className={`flex-1 p-4 sm:p-6 overflow-x-hidden transition-all duration-300 ${
          ...
          <h1 className={`text-2xl sm:text-3xl font-bold ${textClass} mb-4 sm:mb-6 flex items-center`}>
            ...
            Tableau de bord Super-Grossiste
          </h1>
