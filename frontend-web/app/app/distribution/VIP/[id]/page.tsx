...
export default function ConfirmDistributionPage() {
  ...
  useEffect(() => {
    const fetchDistributionInfo = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/vip-tickets/distro/info/${distributionId}`, {
          method: 'GET',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const confirmResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/vip-tickets/distro/conf/${distributionId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
      ...
      setSuccess(`Confirmation réussie ! ${confirmData.nombre_tickets} ticket(s) VIP ont été ajoutés à votre compte.`);
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-xl shadow-lg">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Confirmation de réception des tickets VIP
          </p>
        </div>

        {distributionInfo && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h3 className="font-medium text-blue-900 mb-2">Détails de la distribution VIP</h3>
            <div className="text-sm text-blue-800">
              <p><span className="font-medium">Nombre de tickets VIP :</span> {distributionInfo.nombre_tickets}</p>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div>
            <p className="text-sm text-gray-700 mb-4 text-center">
              Pour confirmer la réception de ces tickets VIP, veuillez vous connecter avec vos identifiants :
            </p>
          </div>
